# Wiki Index

Last updated: 2026-05-23 | Pages: 135

> This file is maintained by the LLM. Do not edit manually.
> The LLM reads this first when answering queries to locate relevant pages.

---

## Overview
| Page | Summary |
|------|---------|
| [[wiki/overview]] | High-level synthesis of everything accumulated in the wiki |

---

## Sources
| Page | Summary | Date Ingested |
|------|---------|---------------|
| [[wiki/sources/openclaw-lex-fridman-491]] | Lex Fridman #491 — Peter Steinberger on OpenClaw: origin story, agentic engineering philosophy, security, MCPs vs skills, acquisition talks | 2026-04-18 |
| [[wiki/sources/dylan-patel-token-supply-demand]] | Invest Like The Best — Dylan Patel on the explosive supply and demand of AI tokens; compute bottlenecks; scaling laws; phantom GDP; model hoarding; robotics | 2026-04-24 |
| [[wiki/sources/dario-amodei-end-of-exponential]] | Dwarkesh Patel — Dario Amodei on end of the exponential, biological revolution, LLM path vs agent-competition path, diffusion, Anthropic culture, AGI timelines | 2026-04-25 |
| [[wiki/sources/openai-founders-core-memory]] | Core Memory — Sam Altman + Greg Brockman on ChatGPT origin, personal AGI vision, Elon Musk trial, compute strategy | 2026-04-25 |
| [[wiki/sources/sundar-pichai-google-ai-history]] | Elad Gil + Patrick Collison — Sundar Pichai on Google AI history, Search-as-agent-manager, speed philosophy, enterprise diffusion barriers, 2027 inflection | 2026-04-25 |
| [[wiki/sources/andrej-karpathy-summoning-ghosts]] | Dwarkesh Patel — Karpathy on LLMs as ghosts, three coding modes, cognitive deficits, education philosophy, research taste, nanochat | 2026-04-25 |
| [[wiki/sources/dylan-patel-compute-bottleneck]] | Dwarkesh Patel — Dylan Patel on energy as 1-year bottleneck, chips as 3-year bottleneck, Huawei threat, Taiwan risk, centralised robot intelligence | 2026-04-25 |
| [[wiki/sources/elon-musk-ai-space]] | Dwarkesh Patel — Elon Musk on space AI compute in 36 months, TeraFab, energy bottleneck, ideas travel in 6 months, xAI hardware-first strategy | 2026-04-25 |
| [[wiki/sources/ilya-sutskever-age-of-research]] | Dwarkesh Patel — Ilya Sutskever on age of scaling ending, age of research beginning, continual learning, AI diversity, research taste, SSI | 2026-04-25 |
| [[wiki/sources/jensen-huang-nvidia-moat]] | Dwarkesh Patel — Jensen Huang on Nvidia's architectural moat, CUDA, China export controls, premium-latency token market, Groq acquisition | 2026-04-25 |
| [[wiki/sources/satya-nadella-microsoft-agi]] | Dwarkesh Patel — Satya Nadella on trust as competitive advantage, AI sovereignty, open source as concentration check, US-China AI race | 2026-04-25 |
| [[wiki/sources/vaibhav-raj-shamani-ai-business]] | Raj Shamani FO499 (Hindi) — Vaibhav on AI SOPs as new resume, skill.md files, claude workflows, AI-first solopreneur business in India | 2026-04-25 |
| [[wiki/sources/leopold-aschenbrenner-2027-agi]] | Dwarkesh Patel — Leopold Aschenbrenner on 2027 AGI, intelligence explosion, US-China race, compute cluster geography, Manhattan Project analogy | 2026-04-25 |
| [[wiki/sources/marc-andreessen-real-ai-boom]] | Dylan Patel — Marc Andreessen on AI diffusion resistance: cartels, unions, government monopolies, founder+AI formula, Elon method | 2026-04-25 |
| [[wiki/sources/marc-andreessen-openclaw-agents]] | Marc Andreessen on OpenClaw, agent bank accounts, AI+crypto grand unification, death of browser, future of abstractions | 2026-04-25 |
| [[wiki/sources/peter-steinberger-clawd-ships-code]] | Peter Steinberger second interview: read prompts not PRs, agentic onboarding, close-the-loop principle, PSPDFKit origin | 2026-04-25 |
| [[wiki/sources/dylan-patel-trillion-dollar-buildout]] | Dylan Patel: RL environments early innings, multimodal mid-innings, text late innings; SaaS business model disruption; embodiment thesis | 2026-04-25 |
| [[wiki/sources/lex-fridman-state-of-ai-2026]] | Lex Fridman #490 with Sebastian Raschka + Nathan Lambert: AI slop, authenticity premium, consciousness, three coding modes | 2026-04-25 |
| [[wiki/sources/jensen-huang-lex-fridman-494]] | Lex Fridman #494 — Jensen Huang: biology unlocked in 5 years, succession philosophy, consciousness upload vision, humanity optimism | 2026-04-25 |
| [[wiki/sources/glp1-health-revolution]] | GLP-1s and the trillion-dollar health revolution: cardiovascular, addiction, obesity; three friction barriers; connects to biological-revolution | 2026-04-25 |
| [[wiki/sources/palmer-luckey-jre-2394]] | JRE #2394 — Palmer Luckey on Oculus origin, Anduril defense AI, non-humanoid combat robots, VR+AI training, multi-sensor UAP verification | 2026-05-17 |
| [[wiki/sources/marc-andreessen-world-malleable]] | Founders Podcast — Marc Andreessen on founder vs manager thesis, bourgeois capitalism vs managerialism, zero introspection, Elon Musk as Henry Ford | 2026-05-17 |
| [[wiki/sources/gavin-baker-gpu-economics]] | Invest Like The Best — Gavin Baker on Blackwell transition, reasoning models saving AI, TPU economics, two new post-training scaling laws, ROI on AI | 2026-05-17 |
| [[wiki/sources/elon-musk-lex-fridman-400]] | Lex Fridman #400 — Elon Musk on war philosophy, AGI as civilisational risk, US-China Thucydides Trap, nuclear risk, ignorance as the real enemy | 2026-05-17 |
| [[wiki/sources/gokul-rajaram-ai-product-development]] | Invest Like The Best — Gokul Rajaram on non-deterministic software, judgment as future-proof skill, AI impact on PMs/designers, SaaS durability taxonomy | 2026-05-17 |
| [[wiki/sources/brian-chesky-airbnb-ai-era]] | Invest Like The Best — Brian Chesky on AI Founder Mode, consumer-AI renaissance, Project Hawaii small teams, 11-Star Experience, home→person atomic unit, apps→agents | 2026-05-17 |
| [[wiki/sources/krishna-rao-anthropic-compute]] | Invest Like The Best — Anthropic CFO Krishna Rao: $9B→$30B Q1 revenue, $100B+ compute commitments, cone of uncertainty, three-chip strategy, Mythos phased release, virtual collaborator | 2026-05-17 |
| [[wiki/sources/marc-andreessen-jre-2501]] | JRE #2501 — Marc Andreessen: Flock AI surveillance, "AGI already here", sand-into-thought, AI vampire/bots-managing-bots, LLMs-write-Netflix-scripts safety reframe, US-China values race, robots & abundance | 2026-05-23 |
| [[wiki/sources/reiner-pope-training-serving]] | Dwarkesh blackboard — Reiner Pope on how LLMs are served: batch size ~1000x lever, optimal batch ≈300×sparsity, MoE-on-rack all-to-all, pipeline parallelism, ~100x over-training vs Chinchilla | 2026-05-23 |
| [[wiki/sources/reiner-pope-chip-design]] | Dwarkesh blackboard — Reiner Pope building an AI chip bottom-up: gates→systolic arrays→FPGA/ASIC→cache vs scratchpad→GPU≈many tiny TPUs; compute-vs-communication | 2026-05-23 |
| [[wiki/sources/jane-street-gpus-trading]] | Dwarkesh at Jane Street — Ron Minsky & Dan Pontecorvo on many trading time-horizons, $6B CoreWeave deal, FPGA determinism, "AGI-complete" trading, data-center engineering, hiring | 2026-05-23 |
| [[wiki/sources/harvard-dropouts-nvidia-challenger]] | Invest Like The Best — Etched (Uberti & Wachen): inference ASIC, low-voltage inference, cluster-scale memory, Sohu, $800M raised, "production is the product", 40-day silicon-to-rack | 2026-05-23 |
| [[wiki/sources/gavin-baker-watts-wafers]] | Invest Like The Best — Gavin Baker on watts (orbital compute) vs wafers (TSMC as bubble-preventer), Terafab, Pareto frontier, usage-based pricing, bitter lesson & continual-learning risks, "different AND hard" chips | 2026-05-23 |
| [[wiki/sources/eric-jang-alphago-selfplay]] | Dwarkesh blackboard — Eric Jang rebuilds AlphaGo (~$10K): MCTS, self-play as improvement operator, why MCTS doesn't fit LLMs, RL info-inefficiency, distillation bits/sample, automated research | 2026-05-23 |

---

## Entities
| Page | Type | Summary |
|------|------|---------|
| [[wiki/entities/peter-steinberger]] | Person | Creator of OpenClaw and PSPDFKit; proponent of agentic engineering; in acquisition talks with Meta and OpenAI |
| [[wiki/entities/openclaw]] | Product | Open-source personal AI agent; fastest-growing GitHub repo; 180k+ stars; self-modifying; TypeScript |
| [[wiki/entities/moltbook]] | Product | AI-agents social network built on OpenClaw; went viral; mediated AI psychosis discussion |
| [[wiki/entities/pspdfkit]] | Organisation | Peter's prior company; PDF SDK on 1 billion devices; sold after 13 years |
| [[wiki/entities/dylan-patel]] | Person | Founder of SemiAnalysis; leading independent analyst of AI compute economics; $7M/yr token spend at his own firm |
| [[wiki/entities/semianalysis]] | Organisation | Independent AI infrastructure research firm; chip reverse engineering; energy grid modelling; tokenomics analysis |
| [[wiki/entities/anthropic]] | Organisation | Frontier AI lab; maker of Claude; $40B+ ARR; compute-constrained; Mythos internally held; 72%+ gross margins |
| [[wiki/entities/dario-amodei]] | Person | CEO of Anthropic; biological revolution thesis; DVQ culture; "end of the exponential" framing; ~10–30% p(catastrophe) |
| [[wiki/entities/sam-altman]] | Person | CEO of OpenAI; ChatGPT origin story; personal AGI vision; Elon Musk trial context |
| [[wiki/entities/greg-brockman]] | Person | Co-founder of OpenAI; "too much opportunity" framing; UX of re-explaining context to AI |
| [[wiki/entities/openai]] | Organisation | Maker of ChatGPT; GPT-4; compute-advantaged vs Anthropic; Elon Musk legal battle; personal AGI roadmap |
| [[wiki/entities/sundar-pichai]] | Person | CEO of Google/Alphabet; Search-as-agent-manager; speed philosophy; 2027 enterprise inflection prediction |
| [[wiki/entities/google]] | Organisation | Invented Transformer; makes Gemini and TPUs; runs Waymo; exploring data centers in space |
| [[wiki/entities/andrej-karpathy]] | Person | Independent AI educator; former OpenAI + Tesla; LLMs as ghosts; three coding modes; education philosophy |
| [[wiki/entities/ilya-sutskever]] | Person | OpenAI co-founder; founder SSI; age of scaling ending; age of research; continual learning; research taste |
| [[wiki/entities/jensen-huang]] | Person | CEO Nvidia; architectural moat; CUDA; Groq acquisition; premium-latency tokens; China export controls |
| [[wiki/entities/nvidia]] | Organisation | Dominant GPU/AI accelerator; CUDA ecosystem; Mellanox; Groq; central to all AI compute supply chains |
| [[wiki/entities/elon-musk]] | Person | CEO xAI; original OpenAI co-founder; TeraFab; space compute; hardware-first AI strategy |
| [[wiki/entities/satya-nadella]] | Person | CEO Microsoft; trust-as-moat thesis; AI sovereignty; OpenAI partner; 2027 enterprise inflection |
| [[wiki/entities/leopold-aschenbrenner]] | Person | Former OpenAI safety; author Situational Awareness; 2027 AGI thesis; intelligence explosion; US-China race |
| [[wiki/entities/marc-andreessen]] | Person | Co-founder a16z; AI diffusion resistance thesis; founder+AI formula; agent bank accounts; AI+crypto grand unification; managerialism collapse thesis |
| [[wiki/entities/palmer-luckey]] | Person | Founder of Oculus and Anduril Industries; defense AI; non-humanoid autonomous weapons; VR+AI combat training |
| [[wiki/entities/gavin-baker]] | Person | Technology investor; encyclopedic AI infrastructure knowledge; reasoning models saved AI; lowest-cost token producer thesis |
| [[wiki/entities/gokul-rajaram]] | Person | Product executive (Google, Facebook, Square, DoorDash); non-deterministic software thesis; judgment as future-proof skill |
| [[wiki/entities/brian-chesky]] | Person | Co-founder/CEO Airbnb; coined "AI Founder Mode"; 11-Star Experience; pipeline recruiting; home→person atomic unit |
| [[wiki/entities/airbnb]] | Organisation | $134B GMV marketplace; AI redesign in progress; Project Hawaii small-team pattern; 50–70 vertical ambition; identity-as-atomic-unit |
| [[wiki/entities/krishna-rao]] | Person | CFO Anthropic; ex-Airbnb, ex-Blackstone PE; raised $75B+ since joining; cone-of-uncertainty + ROI-on-compute frame; 30–40% of time on compute |
| [[wiki/entities/flock]] | Organisation | a16z AI public-safety company; real-time vehicle ID from city cameras (incl. plateless); disabled by Austin, central AI-surveillance case |
| [[wiki/entities/reiner-pope]] | Person | CEO of MatX (AI chip startup); ex-Google TPU architecture; blackboard lectures on chip design and inference economics; "compute vs communication" |
| [[wiki/entities/jane-street]] | Organisation | Quant trading firm; $6B CoreWeave deal; FPGA/GPU/ASIC across many time-horizons; "AGI-complete" trading; heavy ML hiring |
| [[wiki/entities/etched]] | Organisation | Inference ASIC startup (Sohu chip); Harvard-dropout founders; ~$800M raised; low-voltage inference + cluster-scale memory; "production is the product" |
| [[wiki/entities/eric-jang]] | Person | AI researcher (ex-1X VP AI, ex-DeepMind Robotics); rebuilt AlphaGo (~$10K); self-play RL & MCTS; RL info-inefficiency thesis |

---

## Concepts
| Page | Summary |
|------|---------|
| [[wiki/concepts/agentic-engineering]] | Disciplined practice of directing AI agents with architectural context and empathy; contrasted with "vibe coding" |
| [[wiki/concepts/agentic-loop]] | Core runtime pattern of any agent system: input → LLM+tools → execute → repeat → respond |
| [[wiki/concepts/mcps-vs-skills]] | CLIs + skills beat MCPs because they compose naturally, don't pollute context, and use weights the model already knows |
| [[wiki/concepts/personal-agents]] | AI with persistent access to your data/devices; 2026 convergence from Peter, Sam Altman, and Sundar Pichai on the same vision |
| [[wiki/concepts/ai-psychosis]] | Public tendency to uncritically trust or catastrophically fear LLM outputs; also: Claude Code psychosis (productivity-euphoric variant) |
| [[wiki/concepts/soul-md]] | Persistent agent identity document; written partly by the agent itself; philosophical anchor and personality source |
| [[wiki/concepts/prompt-injection]] | Open security problem: malicious content in agent inputs overriding goals; mitigated by smart models, sandboxing, allow-lists |
| [[wiki/concepts/tokenomics]] | Economics of AI token supply and demand; who gets access, at what price, what value they generate; every supply layer sold out |
| [[wiki/concepts/ai-permanent-underclass]] | Dylan Patel's framing: those who fail to use/generate/capture value from tokens will be structurally disadvantaged |
| [[wiki/concepts/phantom-gdp]] | Economic value created by AI productivity that is invisible to standard GDP statistics; coined by SemiAnalysis economist Malcolm |
| [[wiki/concepts/scaling-laws]] | More compute into model makes model better; still holding as of 2026; Mythos (L6 engineer) + Dario's exponential thesis as proof |
| [[wiki/concepts/ai-diffusion]] | The propagation of AI capability into economic/org use; capability and diffusion are two separate curves; 2027 enterprise inflection |
| [[wiki/concepts/biological-revolution]] | Dario Amodei's thesis: AI compresses decades of biomedical progress into years; primary motivating vision for Anthropic's mission |
| [[wiki/concepts/llm-nature]] | Karpathy's reframe: LLMs are ghosts summoned from the human corpus, not novel intelligences; explains capabilities and deficits |
| [[wiki/concepts/post-scaling-research]] | Ilya's thesis: after the internet data wall, algorithmic insight (not compute) becomes the differentiating skill; age of research |
| [[wiki/concepts/ai-sovereignty]] | Geopolitical dimension of AI deployment: trust, data residency, concentration risk, open source as structural check |
| [[wiki/concepts/agi-timeline]] | Cross-source comparison of AGI timeline predictions: 2027 (Leopold), 1-2yr (Dario), not far (Sam); intelligence explosion implications |
| [[wiki/concepts/non-deterministic-software]] | AI-powered software produces variable outputs for identical inputs; shifts quality ownership from spec-writing to evals; judgment is the remaining human skill |
| [[wiki/concepts/ai-surveillance]] | AI applied to monitoring physical space at scale (Flock cameras, ShotSpotter); crime-solving vs. mass-surveillance/abuse trade-off; extends to robots and wearables |
| [[wiki/concepts/llm-no-drives]] | Andreessen's reframe: LLMs "write Netflix scripts", have no self-preservation; rogue behaviour is primed via doomer-seeded latent space — counter to doomer framings |
| [[wiki/concepts/inference-economics]] | First-principles cost/latency/throughput of serving LLMs: batch size (~1000x lever), roofline, KV cache, MoE-on-rack, PD disaggregation, usage-based pricing |
| [[wiki/concepts/asic-vs-gpu]] | Trade-offs among GPUs, TPUs/ASICs, FPGAs: systolic arrays, low precision, determinism, purpose-built inference chips, "different AND hard" |
| [[wiki/concepts/self-play-rl]] | Self-play + MCTS as a policy-improvement operator (AlphaGo); why it beats LLM policy-gradient RL; distillation bits/sample; off-policy/DAgger |

---

## Analyses
| Page | Date | Summary |
|------|------|---------|
| *(no analyses yet)* | | |
